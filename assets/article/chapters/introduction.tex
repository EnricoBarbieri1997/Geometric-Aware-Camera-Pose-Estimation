...
Camera calibration and camera localization are fundamental problems in Computer Vision, underpinning most, if not all, Computer Visions tasks. Camera calibration consists in estimating the intrinsic parameters of a camera, such as focal length, principal point, and lens distortion coefficients, which are essential for accurate image formation. Camera localization, on the other hand, involves determining the camera's position and orientation in a 3D scene, which is crucial for understanding the spatial relationships between objects in the environment.
Camera calibration can be performed using a variety of techniques, including checkerboard patterns~\cite{Zhang2000}, AprilTags, and other fiducial markers. These methods typically rely on point correspondences between the 3D world and the 2D image plane to estimate the camera's intrinsic parameters. We can talk about autocalibration when the camera is not equipped with a calibration target, and the intrinsic parameters are estimated from the images themselves, often using techniques such as self-calibration or structure-from-motion (SfM).

Camera localization, on the other hand, is often performed using techniques such as pose estimation, which involves estimating the camera's position and orientation relative to a known 3D scene. This can be done using point correspondences, as well as higher-level geometric entities such as lines, planes, and quadrics. The latter are particularly useful in scenarios where point correspondences are sparse or unreliable, as they provide additional geometric constraints that can improve the accuracy of the localization process.

A wide range of applications such as SfM, 3D reconstruction, robotics, and extended reality works on top of those two building blocks. A variety of calibration and localization methods have been proposed, from traditional approaches based on point-wise 3D-2D correspondences~\cite{Zhang2000, Sturm2004}, to more recent techniques that exploit correspondences between higher-level geometric entities such as lines~\cite{guo23vanishing}, conics~\cite{teramoto2002calibration}, planes and quadrics~\cite{Bartoli2005, gaudilliere2017perspective}.
