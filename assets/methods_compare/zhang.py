...
def simulate_camera(noise_std=0.0, num_views=4, debug=1):
    ...
    for i in range(num_views):
        ...
        rvec = np.random.uniform(-0.6, 0.6, 3).astype(np.float32)
        z = np.random.uniform(5.0, 15.0)
...
def print_intrinsics_comparison(K_true, A_est, debug = 1):
    ...
    print(f"Δf: {delta[0]:.6f}, Δuv: {delta[1]:.6f}, Δskew: {delta[2]:.4f}")
...
for noise in np.arange(0.0, 0.04, 0.0005):
    for variant in zhang_variants:
        ...
        for i in range(iterations_count):
            ...
            est_K, _, error, est_Rs, est_Ts = run_calibration(
                obj_pts,
                img_pts,
                image_size=(int(gt_K[0, 2] * 2), int(gt_K[1, 2] * 2))
            )
