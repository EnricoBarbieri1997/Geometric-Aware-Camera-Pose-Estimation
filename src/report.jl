...
function report_error_analysis(report_path, noise_steps; number_of_samples=5, output_path=nothing, save_json=false, method_name="ours")
  reports = deserialize(report_path)
  ...
  if save_json
    ...
    for i in eachindex(noise_steps)
      ...
      push!(json_results, create_single_noise_result(method_name, noise_steps[i], delta_f, delta_uv, delta_skew, success_rate, delta_r, delta_t))
    end
    for report in errored
      if !is_initialized(report)
        continue
      end
      push!(json_results, create_single_noise_result(method_name, report.configuration.noise, [], [], [], [], [], []))
    end
